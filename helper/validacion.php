...
class Validacion
{
    ...
    public function Diferente($campo, $campo2)
    {
        if (self::getValor($campo) === self::getValor($campo2)) {
            $this->errores[$campo] = "$campo no puede ser igual que $campo2";
            return false;
        }
        return true;
    }

    public function Igual($campo, $campo2)
    {
        if (self::getValor($campo) !== self::getValor($campo2)) {
            $this->errores[$campo] = "$campo no puede ser diferente de $campo2";
            return false;
        }
        return true;
    }

    public function Indicativo($campo)
    {
        $regex = "/^[A-Z]{1,2}\d[A-Z]{1,3}$/";

        if (!preg_match($regex, $_POST[$campo])) {
            $this->errores[$campo] = "No cumple el patrón $regex";
            return false;
        }
        return true;
    }
    ...
    /**
     * Comrpueba que la fecha posterior, sea posterior a la fecha previa
     * 
     * @param [DateTimeInmutable] $fechaPrevia
     * @param [DateTimeInmutable] $fechaPosterior
     * @return boolean
     */
    public function ValidaFechaPasada(
        DateTimeImmutable $fechaPrevia,
        DateTimeImmutable $fechaPosterior,
        $campo = "fecha"
    ): bool {

        $lF = new libFechas();

        $diferencia = $fechaPrevia->diff($fechaPosterior);

        if ($diferencia->invert == 0 && $lF->intervalASecs($diferencia) > 0) {
            return true;
        }
        $this->errores[$campo] = "La fecha dada no es posterior";
        return false;
    }
